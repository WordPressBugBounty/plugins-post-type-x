<?php
/**
 * Product shipping template part.
 *
		...
		<table class="<?php echo esc_attr( $table_class ); ?>">
			<tr>
				<?php if ( ! empty( $single_names['product_shipping'] ) ) { ?>
					<td class="shipping-label"><?php echo esc_html( $single_names['product_shipping'] ); ?></td>
				<?php } ?>
				<td>
					<ul>
						<?php
						foreach ( $shipping_values as $i => $shipping_value ) {
							...
							if ( ! empty( $shipping_value ) ) {
								...
								?>
								<li class="<?php echo esc_attr( sanitize_title( $shipping_label ) ); ?>">
									<span class="shipping-label"><?php echo esc_html( $shipping_label ); ?></span>
									<span class="shipping-value">(<?php echo esc_html( $shipping_value ); ?>)</span>
								</li>
								<?php
							}
						}
