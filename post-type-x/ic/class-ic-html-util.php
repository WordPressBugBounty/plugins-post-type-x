...
if ( ! class_exists( 'IC_Html_Util' ) ) {
	...
	class IC_Html_Util {
		...
		function form( $form_name, $form_title, $fields, $buttons = '', $action = '', $method = 'post', $p = true, $before = '', $show_title = true, $ajax_data = array() ) {
			...
			if ( ! empty( $ajax_data ) ) {
				$attr['data-ic_ajax_data'] = wp_json_encode( $ajax_data );
			}
			if ( is_array( $fields ) ) {
		...
		function input( $type, $name, $value, $id = '', $required = 0, $label = null, $comment = null, $p = false, $options = array(), $default_attr = array() ) {
			...
			$preserve_empty_checkbox_value = ! empty( $attr['_ic_preserve_empty_value'] );
			unset( $attr['_ic_preserve_empty_value'] );
			if ( ! empty( $required ) ) {
			...
			if ( ! empty( $options ) ) {
				if ( $type === 'dropdown' ) {
					...
					if ( ! empty( $dropdown_options ) ) {
						...
						if ( ! empty( $attr['multiple'] ) ) {
							...
							if ( ! empty( $placeholder ) && empty( $attr['title'] ) ) {
				...
				if ( $type === 'checkbox' ) {
					...
					if ( empty( $attr['value'] ) && ! $preserve_empty_checkbox_value ) {
						$attr['value'] = '1';
					}
				}
		...
		function number( $name, $value = '', $unit = '', $step = null, $min = null, $max = null, $class = '', $id = '', $required = 0, $attr = array() ) {
			$attr = array_merge(
				array(
					'step'  => $step,
					'min'   => $min,
			...
			return $unit . $this->input( 'number', $name, $value, $id, $required, null, null, false, array(), $attr );
		}
		...
		/**
		 * Checks if two option values should be treated as the same selection.
		 *
		 ...
		 * @return bool
		 */
		function values_match( $left, $right ) {
		...
		/**
		 * Checks if an option value exists in a selected values array.
		 *
		 ...
		 * @return bool
		 */
		function has_selected_value( $option_value, $selected_values ) {
			foreach ( $selected_values as $selected_value ) {
				if ( $this->values_match( $selected_value, $option_value ) ) {
					return true;
				}
			}
		...
		function dropdown_options( $options, $selected_value = null ) {
			...
			foreach ( $options as $option_value => $option_label ) {
				...
				if ( $this->values_match( $selected_value, $option_value ) ) {
					$selected = true;
				} elseif ( is_array( $selected_value ) && $this->has_selected_value( $option_value, $selected_value ) ) {
					$selected = true;
				}
		 ...
		 * @param $id
		 * @param $required
		 * @param bool $preserve_empty_value Whether an explicitly empty option value must remain empty.
		 *
		 * @return void
		 */
		function checkbox( $name, $value, $label, $selected = false, $id = '', $required = 0, $class = '', $preserve_empty_value = false ) {
			$attr = array( 'checked' => $selected );
			if ( ! empty( $class ) ) {
				$attr['class'] = $class;
			}
			if ( $preserve_empty_value ) {
				$attr['_ic_preserve_empty_value'] = true;
			}
			...
			foreach ( $options as $option_value => $option_label ) {
				...
				if ( $type === 'radio' ) {
					...
					if ( $name_array_base ) {
						...
						if ( is_array( $selected_value ) && $this->has_selected_value( $option_value, $selected_value ) ) {
							$selected = true;
						}
					}
					$radio_checkbox_options .= $this->checkbox( $name, $option_value, $option_label, $selected, $option_id, $required, $class, '' === $option_value );
				}
			}
		 ...
		 */
		function dashicon( $type, $title = '', $class = '', $attr = array() ) {
		...
		function tag_closer( $type ) {
			...
			if ( in_array( $type, $no_closer, true ) ) {
				return '';
			}
...
if ( ! class_exists( 'ic_html_util', false ) ) {
	class_alias( 'IC_Html_Util', 'ic_html_util' );
}
