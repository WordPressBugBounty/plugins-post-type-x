...
function start_free_implecode_install() {
    if ( isset( $_GET['free_implecode_install'] ) && ! empty( $_GET['slug'] ) && current_user_can( 'install_plugins' ) ) {
        ...
        if ( $url != 'error' ) {
            ...
            echo '<div id="message error" class="error product-adder-message messages-connect">
				<div class="squeezer">
					<h4><strong>' . sprintf( __( 'Automatic installation is currently unavailable because your website cannot connect to WordPress.org. You can <a href="%s">download the extension manually</a> and install it via Plugins > Add New > Upload Plugin.', 'post-type-x' ), $url ) . '</strong></h4>
				</div>
			</div>';
        }
    }
            ...
            async function ic_epc_newsletter_notice() {
                try {
                    ...
                    document.getElementById('ic-mailer-forms-container').innerHTML =
                        '<div class="ic-notice-error"><?php _e( 'Error loading form. Please try again later.', 'post-type-x' )?></div>';
                }
            }
