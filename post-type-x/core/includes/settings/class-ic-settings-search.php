...
/**
 * Provides settings and docs search utilities.
 */
class IC_Settings_Search {
	...
	/**
	 * Search phrase split into words.
	 *
	 * @var array
	 */
	private $exploded_search_word;

	/**
	 * Hooks the search UI and AJAX handlers.
	 *
	 * @return void
	 */
	...
	/**
	 * Returns the current search placeholder.
	 *
	 * @return string
	 */
	public function placeholder() {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only admin tab routing parameter; sanitized on this line.
	...
	/**
	 * Renders the search form and results container.
	 *
	 * @return void
	 */
	public function search_html() {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only settings search phrase; sanitized on this line.
		$search_word         = isset( $_GET['ic-settings-search'] ) ? sanitize_text_field( wp_unslash( $_GET['ic-settings-search'] ) ) : '';
		$class               = '';
		$search_results_html = '';
		...
		?>
		<form class="ic-settings-search<?php echo esc_attr( $class ); ?>">
			<?php
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only: the current admin query args are mirrored back into the search form as hidden fields.
			foreach ( $_GET as $get_key => $get_value ) {
				if ( in_array( $get_key, array( 'find_option_name', 'ic-settings-search' ), true ) ) {
					continue;
				}
				?>
			...
			<input type="submit" class="button-primary" value="<?php echo esc_attr__( 'Search', 'post-type-x' ); ?>">
		...
		<?php
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Presence-only read that decides whether to autofocus the search box.
		if ( empty( $_GET['find_option_name'] ) && empty( $_GET['ic-settings-search'] ) ) {
			?>
			<script>
		...
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Search results HTML is escaped at render time.
		echo $search_results_html;
	}

	/**
	 * Renders the settings search results wrapper.
	 *
	 * @param string $search_word Search phrase.
	 * @return void
	 */
	public function search_results_html( $search_word ) {
		if ( empty( $search_word ) ) {
			return;
		}
	...
	/**
	 * Renders matching settings results.
	 *
	 * @param string $search_word Search phrase.
	 * @return void
	 */
	public function search_results_settings( $search_word ) {
		if ( empty( $this->points ) ) {
			?>
			<div class="ic-settings-search-empty">
				<?php
				/* translators: %s: Search phrase. */
				printf( esc_html__( "Didn't find any adjustment options for '%s'.", 'post-type-x' ), esc_html( $search_word ) );
				?>
			</div>
			<?php
			return;
		...
		<h2>
			<?php
			/* translators: 1: Number of matching settings, 2: Search phrase. */
			printf( esc_html__( "%1\$s adjustment options found in settings for '%2\$s'", 'post-type-x' ), count( $this->points ), esc_html( $search_word ) );
			?>
			:
		</h2>
		<ul>
			...
			foreach ( $this->points as $option_name => $number ) {
				...
				?>
				<li class="ic-settings-search-result <?php echo esc_attr( $class ); ?>">
					<a href="<?php echo esc_url( $url ); ?>"><?php echo esc_html( $label ); ?></a>
					<?php
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Tooltip HTML is built from escaped values above.
					echo $additional;
					?>
				</li>
				<?php
			...
			if ( count( $this->points ) > 5 ) {
				?>
				<li>
					<div class="button-secondary ic-search-settings-show-button">
						<?php
						/* translators: %s: Number of hidden settings. */
						printf( esc_html__( 'Show %s more related adjustment options', 'post-type-x' ), count( $this->points ) - 5 );
						?>
					</div>
				</li>
				<?php
	...
	/**
	 * Renders matching documentation results.
	 *
	 * @param string $search_word Search phrase.
	 * @return void
	 */
	public function search_results_docs( $search_word ) {
		...
		if ( false !== $transient_result ) {
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Cached docs HTML is escaped at render time.
			echo $transient_result;
			return;
		}
		?>
		<div class="ic-docs-ajax-search-container">
			<div class="ic-docs-search-placeholder"><?php esc_html_e( 'Searching also in docs...', 'post-type-x' ); ?></div>
		</div>
		<script>
			var data = {
				'action': 'ic_search_docs',
				'term': <?php echo wp_json_encode( $search_word ); ?>,
	...
	public function ajax_search_docs() {
		...
		// authorization control. This endpoint backs the plugin settings screens and primes a
		// site transient keyed by the search term, so it must be limited to settings managers.
		...
		$search_word = isset( $_POST['term'] ) ? sanitize_text_field( wp_unslash( $_POST['term'] ) ) : '';
		if ( empty( $search_word ) ) {
			wp_die();
		}
		$transient_name   = 'ic-cat-search-docs-' . sanitize_title( $search_word );
		...
		$request = wp_remote_get( 'https://implecode.com/?ic_docs_api_search=' . rawurlencode( $search_word ), $options );
		...
		if ( ! empty( $data ) && is_array( $data ) ) {
			...
			<h2>
			<?php
				/* translators: 1: Number of matching docs, 2: Search phrase. */
				printf( esc_html__( "%1\$s documentation pages found for '%2\$s'", 'post-type-x' ), count( $data ), esc_html( $search_word ) );
			?>
				:</h2>
			<ul>
				<?php
				foreach ( $data as $key => $doc ) {
					...
					?>
						<li class="ic-docs-search-result<?php echo esc_attr( $class ); ?>">
								<a
									href="
									<?php
									echo esc_url(
									...
									?>
									"
									target="_blank"><?php echo esc_html( $doc['title'] ); ?></a>
							<?php
							// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Tooltip HTML is built from escaped values above.
							echo $additional;
							?>
				...
				if ( count( $data ) > 5 ) {
					?>
					<li>
						<div class="button-secondary ic-search-docs-show-button">
							<?php
							/* translators: %s: Number of hidden docs. */
							printf( esc_html__( 'Show %s more related docs', 'post-type-x' ), count( $data ) - 5 );
							?>
						</div>
					</li>
					<?php
			...
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Docs HTML is escaped at render time.
			echo $results_hml;
		} elseif ( empty( $data ) ) {
			ob_start();
			?>
			<div class="ic-docs-search-empty">
				<?php
				/* translators: %s: Search phrase. */
				printf( esc_html__( "Didn't find any docs or tutorials for '%s'.", 'post-type-x' ), esc_html( $search_word ) );
				?>
			</div>
			<?php
	...
	/**
	 * Outputs highlighting JS for a matched option.
	 *
	 * @return void
	 */
	public function js() {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Existence check only; value is sanitized below.
		$find_option_name = isset( $_GET['find_option_name'] ) ? wp_unslash( $_GET['find_option_name'] ) : '';
		if ( '' === $find_option_name ) {
			return;
		}
		$option_name = sanitize_text_field( rawurldecode( $find_option_name ) );
		?>
		<script>
			jQuery(document).ready(function () {
					var optionName = <?php echo wp_json_encode( $option_name ); ?>;
					var container = jQuery('#implecode_settings');
					var scrollTo = jQuery("[name='" + optionName + "']").first();
					if (scrollTo.length === 0) {
						scrollTo = jQuery("[name^='" + optionName + "[']");
					}
					if (scrollTo.length === 0) {
						scrollTo = jQuery("." + optionName);
					}
					if (scrollTo.length === 0) {
						scrollTo = jQuery("[name^='" + optionName + "']");
					}
	...
	/**
	 * Scores settings against the search phrase.
	 *
	 * @param string $search_word Search phrase.
	 * @return void
	 */
	public function search_settings( $search_word ) {
		if ( empty( $search_word ) ) {
			return;
		}
	...
	/**
	 * Scores a string or array against the current search phrase.
	 *
	 * @param string       $option_name  Option name.
	 * @param string|array $sentence     Searchable text.
	 * @param int          $max_points   Maximum score.
	 * @param bool         $do_shortened Whether to retry with a shortened phrase.
	 * @return void
	 */
	public function search( $option_name, $sentence, $max_points, $do_shortened = true ) {
		...
		} elseif ( ! empty( $this->exploded_search_word ) ) {
	...
	/**
	 * Returns a setting label.
	 *
	 * @param string $option_name Option name.
	 * @return string
	 */
	public function get_label( $option_name ) {
		$registered_settings = $this->get_settings();
	...
	/**
	 * Returns a setting tooltip.
	 *
	 * @param string $option_name Option name.
	 * @return string
	 */
	public function get_tip( $option_name ) {
		$registered_settings = $this->get_settings();
	...
	/**
	 * Builds the admin URL for a matched option.
	 *
	 * @param string $option_name Option name.
	 * @return string
	 */
	public function get_url( $option_name ) {
		$registered_settings      = $this->get_settings();
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only: the current search phrase is preserved in a generated admin URL.
		if ( ! empty( $_GET['ic-settings-search'] ) ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only settings search phrase; sanitized on this line.
			$args['ic-settings-search'] = sanitize_text_field( wp_unslash( $_GET['ic-settings-search'] ) );
		}
	...
	/**
	 * Returns a setting value for searching.
	 *
	 * @param string $option_name Option name.
	 * @return mixed
	 */
	...
	/**
	 * Returns registered settings merged with defaults.
	 *
	 * @return array
	 */
