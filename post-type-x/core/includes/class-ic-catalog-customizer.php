...
/**
 * Registers product catalog customizer settings and controls.
 */
class IC_Catalog_Customizer {

	/**
	 * Registers customizer hooks.
	 */
	...
	/**
	 * Registers base customizer settings.
	 *
	 * @return array
	 */
	...
	/**
	 * Registers integration-specific customizer settings.
	 *
	 * @return array
	 */
	...
	public function sanitize_checkbox( $checked ) {
		return 1 === (int) $checked ? 1 : '';
	}

	/**
	 * Adds single-product label settings.
	 *
		...
		foreach ( $single_names as $key => $names ) {
			...
			$settings[] = array(
				...
				'control' => array(
					...
					'args' => array(
						/* translators: %s: single product field label. */
	...
	/**
	 * Adds product-listing label settings.
	 *
	 * @param array $settings Registered settings.
	 *
	 * @return array
		...
		foreach ( $archive_names as $key => $names ) {
			...
			$settings[] = array(
				...
				'control' => array(
					...
					'args' => array(
						/* translators: %s: archive field label. */
						'label'    => sprintf( __( '%s Label', 'post-type-x' ), $label ),
						'section'  => 'ic_product_catalog_listing',
	...
	public function sections( $wp_customize ) {
		...
		$wp_customize->add_section(
			'ic_product_catalog_integration',
			array(
	...
	/**
	 * Registers customizer controls and settings.
	 *
	 * @param WP_Customize_Manager $wp_customize Customizer manager instance.
	 *
	 * @return void
	...
	public function settings( $wp_customize ) {
		if ( ! is_advanced_mode_forced() ) {
			...
			foreach ( $integration_settings as $settings ) {
				...
				if ( 'color' === $settings['control']['args']['type'] ) {
		...
		foreach ( $settings as $setting ) {
			...
			if ( 'color' === $setting['control']['args']['type'] ) {
		...
		require_once AL_BASE_PATH . '/includes/class-more-catalog-implecode-control.php';
		if ( class_exists( 'More_Catalog_ImpleCode_Control' ) ) {
			$wp_customize->add_control(
				new More_Catalog_ImpleCode_Control(
					$wp_customize,
					'implecode_more',
					array(
						'label'    => __( 'Looking for more options?', 'post-type-x' ),
						'section'  => 'ic_product_catalog_integration',
