...
jQuery( document ).ready(
	function ($) {
		...
		jQuery( '.product-category-widget, .widget_product_categories, .ic-catalog-container' ).on(
			'change',
			'.ic-category-select',
			function () {
		...
		jQuery( 'body' ).on(
			...
			function () {
				if (jQuery( '.responsive-filters' ).is( ':visible' )) {
					...
					jQuery( '.responsive-filters .responsive-filters-section-content' ).find( '.filter-active' ).each(
						function () {
...
function ic_switch_popstate_tabs() {
	...
	if (window.location.hash !== '') {
		...
		jQuery( '.boxed .after-product-details' ).each(
			function () {
	...
	jQuery( '.boxed .after-product-details' ).each(
		function () {
			...
			if (after_product_details_container.hasClass( 'ic_accordion_container' )) {
				after_product_details_container.find( '> div' ).each(
					function () {
				...
				after_product_details_container.find( '.ic_tabs h3' ).each(
					function () {
...
function ic_accordion() {
	jQuery( '.boxed .after-product-details > div' ).each(
		function () {
	...
	jQuery( '.ecommerce-product-catalog' ).on(
		'click',
		'.boxed.responsive .after-product-details .catalog-header',
		function () {
			...
			if (jQuery( this ).hasClass( 'open' )) {
				...
				jQuery( '.boxed.responsive .after-product-details .catalog-header.open' ).each(
					function () {
...
function ic_accordion_initial_hide() {
	jQuery( '.boxed.responsive .after-product-details > div' ).each(
		function () {
	...
	jQuery( '.boxed .after-product-details' ).each(
		function () {
			...
			if ( ! after_product_details_container.hasClass( 'ic_tabs_container' )) {
				...
				after_product_details_container.on(
					'click',
					'.ic_tabs > h3',
					function (e) {
						e.preventDefault();
...
function modern_grid_font_size() {
	var max_width = 0;
	jQuery( '.modern-grid-element' ).each(
		function () {
		...
		if (jQuery( '.responsive-filters' ).length === 0) {
			...
			if (responsive_filters === undefined && jQuery( '#product_filters_bar .filter-widget' ).length > 0) {
				if (jQuery( '#product_filters_bar .filter-widget:not(.ic-empty-filter):not(.widget_product_categories):not(.product_search)' ).length > 0) {
					...
					jQuery( '#product_filters_bar .filter-widget:not(.ic-empty-filter)' ).each(
						function () {
			...
			.dialog(
				{
					...
					closeOnEscape: false,
					open: function () {
						jQuery( this ).dialog( 'widget' ).find( '.ui-dialog-titlebar-close' ).remove();
					},
					buttons: {
						Yes: function () {
