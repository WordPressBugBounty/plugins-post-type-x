...
jQuery( document ).ready(
	function () {
		...
		if ( ! isMobile) {
			jQuery( '.sort-settings tbody' ).sortable(
				{
					...
					update: function () {
						jQuery( '.sort-settings tbody tr' ).each(
							function () {
			...
			var icToggleShippingAddressButtons = function (context) {
				var container = context ? jQuery( context ) : jQuery( document.body );
				var columns = container.find( '.product-shipping-address-column' );
				if (container.is( '.product-shipping-address-column' )) {
					columns = columns.add( container );
				}

				columns.each(
					function () {
						var column = jQuery( this );
						var toggle = column.find( '.ic-shipping-address-toggle' );
						var dependents = column.find( '.ic-shipping-address-toggle-dependent' );

						if ( ! toggle.length || ! dependents.length ) {
							return;
						}
			...
			icToggleShippingAddressButtons();

			jQuery( document ).on(
				'change',
				'.ic-shipping-address-toggle',
				function () {
					icToggleShippingAddressButtons( jQuery( this ).closest( '.product-shipping-address-column' ) );
				}
			);

			var icInitDefaultsSyncBoxes = function (context) {
				var container = context ? jQuery( context ) : jQuery( document.body );
				var boxes = container.find( '.ic-epc-defaults-sync-box' );

				if (container.is( '.ic-epc-defaults-sync-box' )) {
					boxes = boxes.add( container );
				}
				...
				boxes.each(
					function () {
						...
						if (box.data( 'syncInit' )) {
							return;
						}

						box.data( 'syncInit', true );

						var ajaxUrl         = box.data( 'ajax-url' );
						var nonce           = box.data( 'nonce' );
						var startAction     = box.attr( 'data-start-action' );
						var statusAction    = box.attr( 'data-status-action' );
						var dirtySelector   = box.attr( 'data-dirty-selector' ) || '';
						var startErrorText  = box.attr( 'data-start-error-text' ) || 'Unable to start the overwrite action.';
						var statusErrorText = box.attr( 'data-status-error-text' ) || 'Unable to read overwrite progress.';
						var doneText        = box.attr( 'data-done-text' ) || 'The overwrite is done.';
						var processedText   = box.attr( 'data-processed-text' ) || 'items processed while overwriting';
						var dirtyText       = box.attr( 'data-dirty-text' ) || 'Save the settings above before starting an overwrite.';
						var spinner         = box.find( '.ic-epc-defaults-sync-spinner' );
						var message         = box.find( '.ic-epc-defaults-sync-message' );
						var buttons         = box.find( '.ic-epc-defaults-sync-button' );
						var submitButtons   = box.find( '.ic-epc-defaults-sync-submit' );
						var triggerButtons  = buttons.length ? buttons : submitButtons;
						var actionOptions   = box.find( '.ic-epc-defaults-sync-action-option' );
						var includeEmpty    = box.find( '.ic-epc-defaults-sync-include-empty' );
						var settingsForm    = box.closest( 'form' );
						var settingsInputs  = dirtySelector ? settingsForm.find( dirtySelector ) : jQuery();
						var pollTimer       = null;
						var isDirty         = false;
						var noSelectionText = box.attr( 'data-no-selection-text' ) || 'Select at least one option to overwrite.';

						function syncButtonState(isWorking) {
							triggerButtons.prop( 'disabled', isWorking );
							triggerButtons.toggleClass( 'disabled', isDirty );
							triggerButtons.toggleClass( 'button-disabled', isDirty );
							triggerButtons.attr( 'aria-disabled', isDirty ? 'true' : 'false' );
							if (isWorking) {
						...
						function showMessage(text, isError) {
							...
							message.toggleClass( 'notice-error', !! isError );
						}

						function getResponseData(response, requireSuccess) {
							if ( ! response || ! jQuery.isPlainObject( response.data )) {
								return false;
							}

							if (requireSuccess !== false && response.success !== true) {
								return false;
							}
						...
						function getResponseText(data, key, fallback) {
						...
						function normalizeStatusData(data) {
							if ( ! data || typeof data.done === 'undefined' ) {
								return false;
							}

							var done = parseInt( data.done, 10 );
							...
							return {
								...
								hasRequest: data.has_request === true || data.has_request === 1,
								inProgress: data.in_progress === true || data.in_progress === 1,
								actionLabel: typeof data.action_label === 'string' ? data.action_label : ''
							};
						}
						...
						function pollStatus() {
							jQuery.post(
								...
								function (response) {
									...
									var statusData   = normalizeStatusData( responseData );
									if ( ! statusData) {
										stopPolling();
										showMessage( statusErrorText, true );
										return;
									}

									if (statusData.hasRequest || statusData.inProgress) {
										showMessage( statusData.done + ' ' + processedText + ' ' + statusData.actionLabel + '.', false );
										schedulePoll();
										return;
									}

									stopPolling();
									showMessage( doneText, false );
								}
							).fail(
								function () {
									stopPolling();
						...
						settingsInputs.on( 'input change', markDirty );

						function startSync(requestData, workingText) {
							stopPolling();
							...
							jQuery.post(
								...
								function (response) {
									var responseData = getResponseData( response, false );
									if ( ! responseData) {
										stopPolling();
										showMessage( startErrorText, true );
										return;
									}

									if (response.success !== true) {
										stopPolling();
										showMessage( getResponseText( responseData, 'message', startErrorText ), true );
										return;
									}

									showMessage( getResponseText( responseData, 'message', startErrorText ), false );
									schedulePoll();
								}
							).fail(
								function (xhr) {
									var errorMessage = startErrorText;
									var responseData = getResponseData( xhr.responseJSON, false );
									if (responseData) {
										errorMessage = getResponseText( responseData, 'message', startErrorText );
									}
									stopPolling();
						...
						buttons.on(
							...
							function (event) {
								...
								var syncAction = button.data( 'sync-action' );

								if ( ! syncAction || isDirty) {
									if (isDirty) {
										event.preventDefault();
										showMessage( dirtyText, true );
										window.alert( dirtyText );
									}
									return;
								}
								...
								startSync(
									{
										...
										include_empty: includeEmpty.is( ':checked' ) ? 1 : 0
									},
									button.text() + '...'
								);
							}
						...
						submitButtons.on(
							...
							function (event) {
								var button = jQuery( this );
								var syncActions = actionOptions.filter( ':checked' ).map(
									function () {
								...
								if ( ! syncActions.length) {
									event.preventDefault();
									showMessage( noSelectionText, true );
									return;
								}
		...
		*/
		...
		jQuery( '.implecode-review-thanks' ).on(
			'click',
			'.dashicons-yes',
			function () {
		...
		*/
		jQuery( '.ic-notice.is-dismissible' ).on(
			'click',
			'.notice-dismiss, .ic-user-dismiss',
			function (event) {
		...
		jQuery( '.implecode-admin-media-image, .blocks-widgets-container' ).on(
			'click',
	...
	function ($) {
		$.ic = {
			...
			doHook: function (hookType, action, value, args) {
				if (undefined != jQuery.ic.hooks[hookType][action]) {
					var hooks = jQuery.ic.hooks[hookType][action];
					for (var i = 0, hooksLength = hooks.length; i < hooksLength; i++) {
						if ('action' == hookType) {
